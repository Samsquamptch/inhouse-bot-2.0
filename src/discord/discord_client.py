...
def run_discord_bot():
    ...
    #         user_check = discord_service.check_for_value("disc", ctx.author.id)
    #     elif '<@' == user[0:2]:
    #         user_acc = await ctx.guild.fetch_member(user[2:-1])
    #         user_check = discord_service.check_for_value("disc", int(user[2:-1]))
    #     else:
    #         user_check, user_acc = check_user.user_exists(ctx.guild, user)
    #     chosen_server = next((x for x in server_list if x.server == ctx.guild.id), None)
    #     if chosen_server is None:
    ...
    #     disc_reg = discord_service.check_for_value("disc", ctx.author.id)
    #     steam_reg = discord_service.check_for_value("steam", dotabuff_id)
