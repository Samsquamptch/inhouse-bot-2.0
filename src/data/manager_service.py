...
def create_tables():
    conn = db_access.get_db_connection()
    conn.cursor().execute("""CREATE TABLE IF NOT EXISTS User(Id INTEGER PRIMARY KEY AUTOINCREMENT, Discord BIGINT UNIQUE, Steam BIGINT UNIQUE,
        MMR INT, Pos1 INT, Pos2 INT,Pos3 INT, Pos4 INT, Pos5 INT, LastUpdated DATETIME)""")
    conn.cursor().execute("""CREATE TABLE IF NOT EXISTS Server(Id INTEGER PRIMARY KEY AUTOINCREMENT, Server BIGINT, AdminChannel BIGINT,
    ...
    conn.cursor().execute("""CREATE TABLE IF NOT EXISTS SteamLogin(ServerId INT UNIQUE, Title CHAR, FOREIGN KEY(ServerId) REFERENCES Server(Id))""")
    conn.cursor().execute("""CREATE TABLE IF NOT EXISTS AutoLobby(MatchId INTEGER PRIMARY KEY AUTOINCREMENT, ServerId INT,
        LobbyStatus BOOL, MatchStatus BOOL, FOREIGN KEY(ServerId) REFERENCES Server(Id))""")
